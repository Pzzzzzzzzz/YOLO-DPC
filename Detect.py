...
if __name__ == '__main__':
    model = YOLO(r'runs\train\exp3\weights\best.pt') # select your model.pt path
    model.predict(
        source=r'color_datasets\images\test',
        save=True,  # 保存预测结果
        imgsz=640,  # 输入图像的大小，可以是整数或w，h
        conf=0.25,  # 用于检测的目标置信度阈值（默认为0.25，用于预测，0.001用于验证）
        iou=0.2,  # 非极大值抑制 (NMS) 的交并比 (IoU) 阈值
